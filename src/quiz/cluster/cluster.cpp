...
void clusterHelper (int i,const std::vector<std::vector<float>> points,std::vector<int> &cluster,std::vector<bool> prcoess, KdTree* tree, float distanceTol)
{
	prcoess[i]=tree;
	cluster.push_back(i);
	std::vector<int> p=tree->search(points[i],distanceTol);
	//std::cout<<"search size"<<p.size()<<std::endl;
	for (int id:p)
	{
	if (!prcoess[i])
	clusterHelper(id,points,cluster,prcoess,tree,distanceTol);
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<bool> process(points.size(),false);
    int i=0;
	while(i<points.size())
	{
		if (process[i])
		...
		clusterHelper(i,points,cluster,process,tree,distanceTol);
...
int main ()
{
	...
	std::cout<<endl;
    std::cout<<"Test Search Finish"<<std::endl;
